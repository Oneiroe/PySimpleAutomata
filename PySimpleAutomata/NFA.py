...
def nfa_intersection(nfa_1: dict, nfa_2: dict) -> dict:
    """ Returns a NFA that reads the intersection of the of the
    NFAs in input.
    ...
    """
    intersection = {
        'alphabet': nfa_1['alphabet'].intersection(
            nfa_2['alphabet']),
        'states': set(
...
def nfa_union(nfa_1: dict, nfa_2: dict) -> dict:
    ...
    union = {
        ...
        'initial_states': nfa_1['initial_states'].union(
            nfa_2['initial_states']),
        'accepting_states': nfa_1['accepting_states'].union(
            nfa_2['accepting_states']),
        'transitions': nfa_1['transitions'].copy()}

    for transition in nfa_2['transitions']:
        for elem in nfa_2['transitions'][transition]:
            union['transitions'].setdefault(transition, set()).add(
                elem)
...
def nfa_interestingness_check(nfa: dict) -> bool:
    """ Checks if the input nfa is interesting, returning
    True/False.
